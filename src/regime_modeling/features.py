import pandas as pd
import numpy as np
from regime_modeling.data.fetcher import fetch_all_data
...
def save_raw_data_cache(merged_df, cache_dir=CACHE_DIR):
    ...
    logger.info(f"✓ Cache saved: {merged_df.shape[0]} rows, {merged_df.shape[1]} columns")
    logger.info(f"  Date range: {merged_df.index.min()} to {merged_df.index.max()}")
    logger.info(f"  Cache timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    return cache_path


def load_raw_data_cache(cache_path=RAW_DATA_CACHE, max_age_hours=CACHE_MAX_AGE_HOURS):
    """
    ...
    logger.info(f"ℹ Cache found:")
    logger.info(f"  Created: {cache_timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"  Age: {cache_age.total_seconds() / 3600:.1f} hours")
    logger.info(f"  Shape: {cache_data['shape']}")
    if 'date_range' in cache_data:
        logger.info(f"  Date range: {cache_data['date_range']['start']} to {cache_data['date_range']['end']}")

    if max_age_hours is not None and cache_age > timedelta(hours=max_age_hours):
        logger.warning(f"⚠ Cache is stale (older than {max_age_hours} hours)")
        return None
...
def get_merged_data(join_type='inner', use_cache=True, force_refresh=False,
    ...
    logger.info("✓ Saved to merged_data.csv")
...
def calculate_volatility(returns_df, window=DEFAULT_VOLATILITY_WINDOW):
    return returns_df.rolling(window=window).std()


def calculate_rsi(prices, period=DEFAULT_RSI_PERIOD):
    delta = prices.diff()
...
def calculate_momentum(prices, period=DEFAULT_MOMENTUM_PERIOD):
...
def get_enhanced_features_for_model(join_type='inner', n_stocks=15, n_indices=5,
                                   ...
                                   include_rsi=True, include_momentum=True,
                                   include_market_breadth=True,
                                   force_refresh=False, cache_max_age_hours=CACHE_MAX_AGE_HOURS):
    """
