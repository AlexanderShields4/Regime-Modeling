...
def analyze_regimes():
    from regime_modeling import setup_logging; setup_logging()
    logger.info("\n" + "="*80)
    logger.info("REGIME ANALYSIS REPORT")
    logger.info("="*80)
    ...
    logger.info(f"Analyzing {len(common_dates)} trading days from {common_dates[0].date()} to {common_dates[-1].date()}\n")
    ...
    for regime_name, count in regime_counts.items():
        ...
        logger.info(f"{regime_name:10s}: {count:4d} days ({pct:5.1f}%)")
    ...
    for regime_name in ['Bull', 'Sideways', 'Bear']:
        ...
        logger.info("-" * 80)
        logger.info(f"Occurrences:     {len(regime_data)} days ({len(regime_data)/len(df_aligned)*100:.1f}% of time)")
        logger.info(f"\nReturns:")
        logger.info(f"  Mean daily:    {mean_return:+.6f} ({mean_return*252*100:+6.1f}% annualized)")
        logger.info(f"  Median daily:  {median_return:+.6f}")
        logger.info(f"  Std deviation: {std_return:.6f} ({std_return*np.sqrt(252)*100:.1f}% annualized)")
        logger.info(f"  Sharpe ratio:  {sharpe:+.3f}")
        logger.info(f"  Min daily:     {daily_returns.min():+.6f} ({daily_returns.min()*100:+.2f}%)")
        logger.info(f"  Max daily:     {daily_returns.max():+.6f} ({daily_returns.max()*100:+.2f}%)")
        logger.info(f"\nDay Distribution:")
        logger.info(f"  Positive days: {positive_days} ({positive_days/len(regime_data)*100:.1f}%)")
        logger.info(f"  Negative days: {negative_days} ({negative_days/len(regime_data)*100:.1f}%)")

        if mean_vol is not None:
            logger.info(f"\nVolatility:")
            logger.info(f"  Avg volatility: {mean_vol:.6f}")

        if mean_momentum is not None:
            logger.info(f"\nMomentum:")
            logger.info(f"  Avg momentum:   {mean_momentum:.6f}")
    ...
    logger.info(f"Total regime changes: {n_transitions}")
    logger.info(f"Average regime duration: {avg_duration:.1f} days")
    transition_counts = {}
    ...
    for from_num in sorted(regime_names_by_num.keys()):
        for to_num in sorted(regime_names_by_num.keys()):
            if from_num != to_num:
                ...
                logger.info(f"  {from_name:10s} → {to_name:10s}: {count:3d} times")

    logger.info("\n" + "="*80)
    logger.info("EXAMPLE PERIODS FOR EACH REGIME")
    logger.info("="*80)
    ...
    for regime_name in ['Bull', 'Sideways', 'Bear']:
        ...
        logger.info(f"Longest {regime_name} periods:")
        for i, (start, end) in enumerate(periods_sorted[:5]):
            ...
            logger.info(f"  {i+1}. {start.date()} to {end.date()} ({duration:3d} days, {period_return*100:+.1f}% cumulative)")
        ...
        for i, (start, end) in enumerate(periods_sorted[-5:][::-1]):
            ...
            period_data = df_aligned.loc[start:end]
            period_return = period_data[return_cols].mean(axis=1).sum()
            logger.info(f"  {start.date()} to {end.date()} ({duration:3d} days, {period_return*100:+.1f}% cumulative)")
    ...
    logger.info(f"  Return features:    {len(return_cols)}")
    logger.info(f"  Volatility features: {len(vol_cols)}")
    logger.info(f"  Momentum features:   {len(momentum_cols)}")
    ...
    logger.info("\n" + "="*80)
    logger.info("PORTFOLIO ALLOCATION STRATEGY")
    logger.info("="*80)
    ...
    logger.info("\n" + "="*80)
    logger.info("Saving detailed report to regime_analysis_report.txt...")
    logger.info("="*80 + "\n")
